(function () {
    var _dataCtrl = function ($interval, $scope, $timeout, countyService, googleChartsService, smogService) {
        var _updateData = function () {
            return smogService.getData($scope.counties.selected).then(function (data) {
        ...
        countyService.getList().then(function (counties) {
        ...
        }).then(_updateData).then(googleChartsService.load).then(function () {
            ...
            $timeout(function () {
                _updateData();
                $interval(_updateData, _toMilis(60));
            }, _nowEndOfHour.diff(_now) + _toMilis(15));
        });
    };
    
    angular.module('controllers').controller('dataCtrl', ['$interval', '$scope', '$timeout', 'countyService', 'googleChartsService', 'smogService', _dataCtrl]);
})();